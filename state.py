...
def create(n):
                            # creating a list of 9 increasing numbers
    ...
    m="<>v^"                # m is "<>v^" - for every possible move (left, right, down, up)
                            # mixing the numbers n*n*n times
...
def hdistance(s):                   # the heuristic value of s
    ...
    for i in range (len(s)):
        if s[i]!=i:
            index=s[i]
            columnGlobal=i/math.sqrt(len(s))
            rowGlobal=i%math.sqrt(len(s))
            columnIndex=index/math.sqrt(len(s))
            rowIndex=index%math.sqrt(len(s))
            manh_dist+=abs(columnGlobal-columnIndex)+abs(rowGlobal-rowIndex)
